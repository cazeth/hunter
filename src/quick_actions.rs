...
impl FoldableWidgetExt for ListView<Vec<QuickActions>> {
    ...
    fn render_header(&self) -> HResult<String> {
        let mime = &self.content.get(0).ok_or(HError::NoneError)?.mime;
...
impl ListView<Vec<QuickActions>> {
    ...
    fn run_action(&mut self, num: Option<usize>) -> HResult<()> {
        ...
        let current_fold = self.current_fold().ok_or(HError::NoneError)?;
        let fold_start_pos = self.fold_start_pos(current_fold);
        let selection = self.get_selection();
...
impl QuickAction {
    ...
    fn run(&self,
        ...
        let cwd = files.get(0).ok_or(HError::NoneError)?.parent_as_file()?;
