...
impl Process {
    fn read_proc(&mut self) -> HResult<()> {
        ...
        std::thread::spawn(move || -> HResult<()> {
            let stdout = handle.lock().stdout.take().ok_or(HError::NoneError)?;
...
impl ListView<Vec<Process>> {
    ...
    fn kill_proc(&mut self) -> HResult<()> {
        let proc = self.selected_proc().ok_or(HError::NoneError)?;
...
impl ProcView {
    ...
    fn show_output(&mut self) -> HResult<()> {
        ...
        let output = self.get_listview_mut().selected_proc().ok_or(HError::NoneError)?.output.lock().clone();
