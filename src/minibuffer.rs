...
impl History {
    ...
    fn get_prev(&mut self, htype: &str) -> HResult<String> {
        self.load()?;
        let history = self.history.get(htype).ok_or(HError::NoneError)?;
...
impl MiniBuffer {
    ...
    pub fn complete(&mut self) -> HResult<()> {
        if !self.input.ends_with(" ") {
            ...
            if let Ok(mut completions) = completions {
                let completion = completions.pop().ok_or(HError::NoneError)?;
    ...
    pub fn cycle_completions(&mut self) -> HResult<()> {
        let last_comp = self.last_completion.as_ref().ok_or(HError::NoneError)?;
        ...
        let next_comp = self.completions.pop().ok_or(HError::NoneError)?;
...
pub fn find_bins(comp_name: &str) -> HResult<Vec<OsString>> {
    ...
    let paths = std::env::var_os("PATH").ok_or(HError::NoneError)?;
...
pub fn find_files(comp_name: &str) -> HResult<Vec<OsString>> {
    ...
    let filename_part = path.file_name().ok_or(HError::NoneError)?;

    let dir = if path.is_dir() { &path } else { path.parent().ok_or(HError::NoneError)? };
