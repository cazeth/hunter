...
impl<T> HBox<T> where T: Widget + PartialEq {
    ...
    pub fn resize_children(&mut self) -> HResult<()> {
        ...
        if self.zoom_active {
            ...
            self.active_widget_mut().ok_or(HError::NoneError)?.set_coordinates(&coords).log();
            return Ok(());
        }
...
impl<T> Widget for HBox<T> where T: Widget + PartialEq {
    ...
    fn render_header(&self) -> HResult<String> {
        self.active_widget().ok_or(HError::NoneError)?.render_header()
    }

    fn refresh(&mut self) -> HResult<()> {
        if self.zoom_active {
            self.active_widget_mut().ok_or(HError::NoneError)?.refresh().log();
            return Ok(());
        }
    ...
    fn get_drawlist(&self) -> HResult<String> {
        if self.zoom_active {
            return self.active_widget().ok_or(HError::NoneError)?.get_drawlist();
        }
    ...
    fn on_event(&mut self, event: Event) -> HResult<()> {
        self.active_widget_mut().ok_or(HError::NoneError)?.on_event(event)?;
        Ok(())
    }

    fn on_key(&mut self, key: termion::event::Key) -> HResult<()> {
        self.active_widget_mut().ok_or(HError::NoneError)?.on_key(key)
    }
}
