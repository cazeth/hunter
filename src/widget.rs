...
pub trait Widget {
    ...
    fn animate_slide_up(&mut self, animator: Option<&Stale>) -> HResult<()> {
        ...
        for i in (0..number_of_frames).rev() {
            ...
            if !animator.as_ref().ok_or(HError::NoneError)?.is_stale()? {
    ...
    fn handle_input(&mut self) -> HResult<()> {
        ...
        let rx_global_event = self.get_core()?.event_receiver.lock().take().ok_or(HError::NoneError)?;
