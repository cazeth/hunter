...
impl Bookmarks {
    ...
    pub fn get(&self, key: char) -> HResult<&String> {
        let path = self.mapping.get(&key).ok_or(HError::NoneError)?;
        Ok(path)
    }
...
impl BMPopup {
    ...
    pub fn pick(&mut self, cwd: String) -> HResult<String> {
        ...
        Ok(bookmark.ok_or(HError::NoneError)?)
    }
...
impl Widget for BMPopup {
    ...
    fn get_drawlist(&self) -> HResult<String> {
        ...
        if !self.add_mode {
            let cwd = self.bookmark_path.as_ref().ok_or(HError::NoneError)?;
    ...
    fn on_key(&mut self, key: Key) -> HResult<()> {
        match key {
            ...
            Key::Char(key) => {
                if self.add_mode {
                    let path = self.bookmark_path.take().ok_or(HError::NoneError)?;
